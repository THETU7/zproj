...
// CPU reference via GDAL/PROJ: EPSG:4326 (lon/lat, degrees) -> EPSG:4978 (ECEF).
...
std::vector<Geodetic> make_points(std::size_t n)
{
    ...
    {
        geo[i] = Geodetic{lat_deg(rng) * kDeg2Rad, lon_deg(rng) * kDeg2Rad, height(rng)};
    }
    return geo;
}
...
int main()
{
    ...
    const double gdal_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
    const double cuda_ms = std::chrono::duration<double, std::milli>(t2 - t1).count();

    std::cout << "max |GDAL - CUDA|   = " << err << " m\n";
    std::cout << "GDAL/PROJ (CPU)     = " << gdal_ms << " ms\n";
    std::cout << "zproj CUDA (GPU)    = " << cuda_ms << " ms  (incl. H2D/D2H + sync)\n";
    if (cuda_ms > 0.0)
    {
