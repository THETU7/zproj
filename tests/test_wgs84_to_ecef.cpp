...
Ecef ReferenceEcef(const Geodetic& g) {
    const double sin_lat = std::sin(g.lat);
    const double cos_lat = std::cos(g.lat);
    const double sin_lon = std::sin(g.lon);
    const double cos_lon = std::cos(g.lon);
    const double n =
        wgs84::kSemiMajorAxis /
        std::sqrt(1.0 - wgs84::kEccentricitySquared * sin_lat * sin_lat);
    return Ecef{(n + g.h) * cos_lat * cos_lon,
                (n + g.h) * cos_lat * sin_lon,
                (n * (1.0 - wgs84::kEccentricitySquared) + g.h) * sin_lat};
}
...
TEST(Wgs84ToEcefCpu, KnownPoints) {
    ...
    std::vector<Geodetic> pts{
        {0.0, 0.0, 0.0},
        {90.0 * kDeg2Rad, 0.0, 0.0},
        {0.0, 90.0 * kDeg2Rad, 0.0},
        {45.0 * kDeg2Rad, 30.0 * kDeg2Rad, 500.0},
    };
