// ztensor/kernel/CopyCPU.cpp
//
// CPU Copy kernel: element-wise cast from src.dtype() to dst.dtype(), honoring
// strides. A double dtype-dispatch expands the per-element read-cast-write for
// the concrete (src_t, dst_t) pair selected from the runtime dtypes. The same-
// dtype + both-contiguous + same-device case is a bytewise Memcpy handled in
// Tensor::copy_ (this kernel is only reached for casts or strided cases).
...
// Per-element copy with cast, templated on (src_t, dst_t). CPU host lambda.
...
#undef ZT_COPY_INNER
    ZT_LOG_ERROR("CopyCPU: unsupported dst dtype {}",
                 toString(dst.scalar_type()));
}

}  // namespace

void CopyCPU(const Tensor& src, const Tensor& dst) { copy_dispatch(src, dst); }
