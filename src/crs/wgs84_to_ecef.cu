...
__global__ void wgs84_to_ecef_kernel(const Geodetic* in,
                                     Ecef* out,
                                     std::size_t n) {
...
std::vector<Ecef> wgs84_to_ecef(const std::vector<Geodetic>& geodetic) {
    ...
    ZPROJ_CUDA_CHECK(cudaMemcpy(d_in,
                                geodetic.data(),
                                geodetic.size() * sizeof(Geodetic),
                                cudaMemcpyHostToDevice));

    wgs84_to_ecef_device(d_in, d_out, geodetic.size());
    ZPROJ_CUDA_CHECK(cudaDeviceSynchronize());

    ZPROJ_CUDA_CHECK(cudaMemcpy(
        out.data(), d_out, out.size() * sizeof(Ecef), cudaMemcpyDeviceToHost));
